...
def test_statistics_container_renders_with_correct_structure(test_db, monkeypatch):
    ...
    assert len(stats_items) == 4, "Should have exactly 4 statistics items"
    
    # Expected statistics in order
    expected_stats = [
        ('Total Cases:', 'totalCases'),
        ('Complete:', 'completePairs'),
        ('Valid:', 'validPairs'),
        ('Done:', 'donePairs')
    ]
    ...
    for i, (expected_label, expected_x_text) in enumerate(expected_stats):
        ...
        assert value_span.text.strip() == '0', \
            f"Default value should be '0', got '{value_span.text.strip()}'"
...
def test_statistics_labels_are_correctly_formatted(test_db, monkeypatch):
    ...
    assert len(label_spans) == 4, "Should have exactly 4 statistics labels"
    
    # Expected labels in order
    expected_labels = ['Total Cases:', 'Complete:', 'Valid:', 'Done:']
...
def test_statistics_alpine_js_directives_are_present(test_db, monkeypatch):
    ...
    assert len(value_spans) == 4, "Should have 4 statistics values"
    
    # Expected x-text directives in order
    expected_directives = ['totalCases', 'completePairs', 'validPairs', 'donePairs']
