...
def simulate_alpine_filter(pairs, filter_str):
    ...
    for pair in pairs:
        ...
        if pair.genomic and not matches:
            if (
                filter_lower in pair.genomic.vorgangsnummer.lower()
                or filter_lower in pair.genomic.meldebestaetigung.lower()
                or filter_lower in pair.genomic.typ_der_meldung.lower()
                or filter_lower in pair.genomic.indikationsbereich.lower()
                or filter_lower in pair.genomic.ergebnis_qc.lower()
                or filter_lower in pair.genomic.source_file.lower()
            ):
                matches = True
        ...
        if pair.clinical and not matches:
            if (
                filter_lower in pair.clinical.vorgangsnummer.lower()
                or filter_lower in pair.clinical.meldebestaetigung.lower()
                or filter_lower in pair.clinical.typ_der_meldung.lower()
                or filter_lower in pair.clinical.indikationsbereich.lower()
                or filter_lower in pair.clinical.ergebnis_qc.lower()
                or filter_lower in pair.clinical.source_file.lower()
            ):
                matches = True
            ...
            for i in range(num_pairs):
                ...
                if record_type in [1, 2]:
                    clinical_record = MeldebestaetigungRecord(
                        ...
                        indikationsbereich=f"indication_{i}",
                        art_der_daten="C",
                        ergebnis_qc=qc_results[i * 2 + 1]
                        if i * 2 + 1 < len(qc_results)
                        else "1",
                        case_id=case_id,
                        gpas_domain="test_domain",
                        processed_at=datetime(2023, 1, 1, 12, 0, 0),
...
def test_done_status_reactivity(
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for i, pair in enumerate(initial_pairs):
            ...
            updated_pair = next(
                (p for p in updated_pairs if p.case_id == pair.case_id), None
            )
            ...
            if updated_pair.is_complete:
                ...
                expected_done = (
                    updated_pair.genomic.is_done and updated_pair.clinical.is_done
                )
            else:
...
@given(
    ...
    sort_columns=st.lists(
        st.sampled_from(
            ["case_id", "typ_der_meldung", "indikationsbereich", "source_file"]
        ),
        min_size=1,
        max_size=5,
    ),
    sort_directions=st.lists(st.sampled_from(["asc", "desc"]), min_size=1, max_size=5),
    record_types=st.lists(
        st.integers(
            min_value=0, max_value=2
        ),  # 0=genomic only, 1=clinical only, 2=both
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for sort_column in sort_columns[:3]:  # Limit to avoid too many iterations
            for sort_direction in sort_directions[:2]:
                ...
                assert (
                    sorted_stats["completePairs"] == initial_stats["completePairs"]
                ), (
                ...
                for initial_pair in initial_pairs:
                    sorted_pair = next(
                        (p for p in sorted_pairs if p.case_id == initial_pair.case_id),
                        None,
                    )
                    assert sorted_pair is not None, (
                        f"Pair {initial_pair.case_id} missing after sorting"
                    )
...
def test_statistics_container_placement_next_to_filter(test_db, monkeypatch):
    ...
    assert filter_stats_container is not None, (
        "filter-and-stats-container should be present"
    )
...
def test_statistics_display_with_empty_dataset(monkeypatch):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert error_div is not None, (
            "Error message should be present for empty dataset"
        )
        assert "No records found" in error_div.text, (
            "Should show 'No records found' message"
        )

        # Statistics container should NOT be present when there's an error message
        stats_container = soup.find("div", class_="statistics-container")
        assert stats_container is None, (
            "Statistics container should NOT be present when there's an error message"
        )
...
def test_statistics_error_handling_javascript_present(test_db, monkeypatch):
    ...
    assert "try {" in html_content, (
        "Error handling try blocks should be present in JavaScript"
    )
    assert "catch (error)" in html_content, (
        "Error handling catch blocks should be present"
    )
...
def test_statistics_safe_property_access_patterns(test_db, monkeypatch):
    ...
    assert "safeString" in html_content, "Should have safe string access function"

    # Check for type checking
    assert "typeof" in html_content, "Should use typeof for type checking"
...
def test_error_handling_with_invalid_json_data(test_db, monkeypatch):
    ...
    assert "try {" in response.text, (
        "Error handling try-catch should be present in JavaScript"
    )
    assert "catch (error)" in response.text, (
        "Error handling catch block should be present"
    )
...
def test_statistics_performance_with_large_dataset(test_db, monkeypatch):
    ...
    monkeypatch.setattr(
        "mvh_copy_mb.web_database.WebDatabaseService.get_all_records_grouped",
    ...
    assert "get totalCases()" in html_content, (
        "Should use computed property for totalCases"
    )
    assert "get readyPairs()" in html_content, (
        "Should use computed property for readyPairs"
    )
    assert "get donePairs()" in html_content, (
        "Should use computed property for donePairs"
    )

    # Check that filtering is efficient (uses single pass)
    assert "filteredAndSorted" in html_content, (
        "Should use cached filtered and sorted data"
    )
...
def test_statistics_reactive_updates_are_immediate(test_db, monkeypatch):
    ...
    for value_span in value_spans:
        ...
        assert "Cases" in x_text or "Pairs" in x_text, (
            "x-text should reference computed properties"
        )

    # Verify that computed properties are properly defined
    assert "get totalCases()" in html_content, (
        "totalCases computed property should be defined"
    )
    assert "get readyPairs()" in html_content, (
        "readyPairs computed property should be defined"
    )
    assert "get donePairs()" in html_content, (
        "donePairs computed property should be defined"
    )

    # Verify that statistics depend on filteredAndSorted (efficient single calculation)
    assert "this.filteredAndSorted" in html_content, (
        "Statistics should use cached filteredAndSorted"
    )
...
def test_statistics_calculations_are_optimized(test_db, monkeypatch):
    ...
    assert totalCases_count >= 3, (
        "All statistics should use the same filteredAndSorted data"
    )
    ...
    assert try_count <= 15, (
        f"Should not have excessive try-catch blocks, found {try_count}"
    )
