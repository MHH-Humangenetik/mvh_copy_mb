...
def test_update_done_status_success(test_db, monkeypatch):
    ...
    response = client.post("/api/done/CASE_COMPLETE", data={"done": "true"})
...
def test_update_done_status_incomplete_pair(test_db, monkeypatch):
    ...
    response = client.post("/api/done/CASE_INCOMPLETE", data={"done": "true"})
...
def test_update_done_status_nonexistent_case_id(test_db, monkeypatch):
    ...
    response = client.post("/api/done/CASE_NONEXISTENT", data={"done": "true"})
...
def test_update_done_status_toggle(test_db, monkeypatch):
    ...
    response1 = client.post("/api/done/CASE_COMPLETE", data={"done": "true"})
    ...
    response2 = client.post("/api/done/CASE_COMPLETE", data={"done": "false"})
...
def test_checkbox_update_flow_integration(test_db, monkeypatch):
    ...
    response2 = client.post("/api/done/CASE_COMPLETE", data={"done": "true"})
    assert response2.status_code == 200
...
@given(
    output_date=st.one_of(
        st.none(), st.dates(min_value=date(2020, 1, 1), max_value=date(2030, 12, 31))
    )
)
...
def test_api_compatibility_with_output_date(test_db, monkeypatch, output_date):
    ...
    **Validates: Requirements 6.5**
    ...
    update_response = client.post("/api/done/CASE_API_TEST", data={"done": "true"})
