...
class TestCLIIntegrationStatistics:
        ...
        for i, row in enumerate(rows):
            ...
            with open(csv_path, "w", newline="", encoding="utf-8") as f:
                writer = csv.DictWriter(
                    f, fieldnames=["Vorgangsnummer", "Meldebestaetigung"], delimiter=";"
                )
                writer.writeheader()
                writer.writerow(row)
    ...
    def create_mock_gpas_client(self, case_id_mapping: dict = None) -> Mock:
        ...
        mock_client.get_original_value.side_effect = lambda psn: case_id_mapping.get(
            psn
        )
        ...
        with tempfile.TemporaryDirectory() as temp_dir_str:
            ...
            with MeldebestaetigungDatabase(db_path) as db:
                for csv_path in csv_paths:
                    process_csv_file(
                        csv_path, temp_dir, mock_gpas_client, db, False, None, stats
                    )
            ...
            assert stats.ready_pairs_count == 1, (
                f"Expected 1 ready pair, got {stats.ready_pairs_count}"
            )
            assert stats.unpaired_genomic_count == 0, (
                f"Expected 0 unpaired genomic, got {stats.unpaired_genomic_count}"
            )
            assert stats.unpaired_clinical_count == 0, (
                f"Expected 0 unpaired clinical, got {stats.unpaired_clinical_count}"
            )
            assert stats.ignored_count == 0, (
                f"Expected 0 ignored files, got {stats.ignored_count}"
            )

            # Verify total calculation (ready pairs counted as 2 files each)
            expected_total = 1 * 2  # 1 ready pair * 2 files
            assert stats.get_total_files() == expected_total, (
                f"Expected total {expected_total}, got {stats.get_total_files()}"
            )
            ...
            assert len(genomic_files) == 1, (
                f"Expected 1 genomic file, got {len(genomic_files)}"
            )
            assert len(clinical_files) == 1, (
                f"Expected 1 clinical file, got {len(clinical_files)}"
            )

            # Verify filenames contain case IDs
            assert "CASE001_HUMGEN" in genomic_files[0].name, (
                "Genomic file should contain case ID"
            )
            assert "CASE001_HUMGEN" in clinical_files[0].name, (
                "Clinical file should contain case ID"
            )
    ...
    def test_cli_workflow_with_unpaired_files_statistics(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir_str:
            ...
            assert stats.ready_pairs_count == 0, (
                f"Expected 0 ready pairs, got {stats.ready_pairs_count}"
            )
            assert stats.unpaired_genomic_count == 0, (
                f"Expected 0 unpaired genomic, got {stats.unpaired_genomic_count}"
            )
            ...
            assert stats.ignored_count == 3, (
                f"Expected 3 ignored files (all unresolved), got {stats.ignored_count}"
            )
            ...
            assert len(genomic_files) == 1, (
                f"Expected 1 NOTFOUND genomic file, got {len(genomic_files)}"
            )
            assert len(clinical_files) == 1, (
                f"Expected 1 NOTFOUND clinical file, got {len(clinical_files)}"
            )
            assert len(unknown_files) == 1, (
                f"Expected 1 NOTFOUND unknown file, got {len(unknown_files)}"
            )
    ...
    def test_cli_workflow_with_ignored_files_statistics(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir_str:
            ...
            assert stats.ignored_count == 3, (
                f"Expected 3 ignored files, got {stats.ignored_count}"
            )
            ...
            assert non_initial_dir1.exists(), (
                "Non-initial directory 1 should be created"
            )
            assert non_initial_dir2.exists(), (
                "Non-initial directory 2 should be created"
            )
            ...
            assert len(qc_failed_files) == 1, (
                f"Expected 1 QC_FAILED file, got {len(qc_failed_files)}"
            )
            assert len(non_initial_files1) == 1, (
                f"Expected 1 NO_INITIAL file in dir1, got {len(non_initial_files1)}"
            )
            assert len(non_initial_files2) == 1, (
                f"Expected 1 NO_INITIAL file in dir2, got {len(non_initial_files2)}"
            )
    ...
    def test_cli_workflow_mixed_scenarios_statistics(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir_str:
            ...
            assert stats.ignored_count == 4, (
                f"Expected 4 ignored files, got {stats.ignored_count}"
            )

            # Verify total calculation
            expected_total = (
                1 * 2 + 0 + 0 + 4
            )  # 1 ready pair * 2 + 0 unpaired G + 0 unpaired C + 4 ignored = 6
            assert stats.get_total_files() == expected_total, (
                f"Expected total {expected_total}, got {stats.get_total_files()}"
            )
    ...
    def test_cli_workflow_with_gepado_enabled_statistics(
        self, mock_validate_update, mock_create_client
    ):
        ...
        with tempfile.TemporaryDirectory() as temp_dir_str:
            ...
            assert stats.unpaired_genomic_count == 1, (
                f"Expected 1 unpaired genomic, got {stats.unpaired_genomic_count}"
            )
            assert stats.unpaired_clinical_count == 0, (
                f"Expected 0 unpaired clinical, got {stats.unpaired_clinical_count}"
            )
            ...
            assert stats.gepado_genomic_updates == 1, (
                f"Expected 1 GEPADO genomic update, got {stats.gepado_genomic_updates}"
            )
            assert stats.gepado_clinical_updates == 0, (
                f"Expected 0 GEPADO clinical updates, got {stats.gepado_clinical_updates}"
            )
            assert stats.gepado_no_updates_needed == 1, (
                f"Expected 1 GEPADO no updates needed, got {stats.gepado_no_updates_needed}"
            )
            assert stats.gepado_errors == 1, (
                f"Expected 1 GEPADO error, got {stats.gepado_errors}"
            )
            ...
            assert mock_validate_update.call_count == 3, (
                f"Expected 3 GEPADO calls, got {mock_validate_update.call_count}"
            )
    ...
    def test_statistics_display_integration(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir_str:
            ...
            assert "PROCESSING SUMMARY" in captured_output, (
                "Should contain summary header"
            )
            assert "Ready pairs:" in captured_output, "Should contain ready pairs count"
            assert "Unpaired genomic:" in captured_output, (
                "Should contain unpaired genomic count"
            )
            assert "Unpaired clinical:" in captured_output, (
                "Should contain unpaired clinical count"
            )
            assert "Ignored files:" in captured_output, (
                "Should contain ignored files count"
            )
            assert "GEPADO OPERATIONS:" in captured_output, (
                "Should contain GEPADO section"
            )
            assert "Updated genomic data:" in captured_output, (
                "Should contain GEPADO genomic updates"
            )
            assert "Updated clinical data:" in captured_output, (
                "Should contain GEPADO clinical updates"
            )
            assert "Errors during ops:" in captured_output, (
                "Should contain GEPADO errors"
            )
    ...
    def test_statistics_display_without_gepado(self):
        ...
        assert "GEPADO OPERATIONS:" not in captured_output, (
            "Should not contain GEPADO section when disabled"
        )
        assert "Updated genomic data:" not in captured_output, (
            "Should not contain GEPADO genomic updates"
        )
        assert "Updated clinical data:" not in captured_output, (
            "Should not contain GEPADO clinical updates"
        )
        assert "Errors during ops:" not in captured_output, (
            "Should not contain GEPADO errors"
        )
    ...
    def test_error_handling_in_statistics_workflow(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir_str:
            ...
            captured_output = output_buffer.getvalue()
            assert "PROCESSING SUMMARY" in captured_output, (
                "Display should work despite errors"
            )
...
class TestStatisticsAccuracyWithRealData:
    ...
    def create_realistic_csv_data(
        self, temp_dir: Path, filename_base: str, scenario: str
    ) -> list:
    ...
    def test_genomic_sequencing_batch_accuracy(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir_str:
            ...
            csv_paths = self.create_realistic_csv_data(
                temp_dir, "genomic_batch", "genomic_sequencing_batch"
            )
            case_id_mapping = self.create_realistic_case_id_mapping(
                "genomic_sequencing_batch"
            )
            ...
            assert stats.unpaired_genomic_count == 3, (
                f"Expected 3 unpaired genomic files, got {stats.unpaired_genomic_count}"
            )
            assert stats.unpaired_clinical_count == 0, (
                f"Expected 0 unpaired clinical files, got {stats.unpaired_clinical_count}"
            )
            assert stats.ignored_count == 5, (
                f"Expected 5 ignored files, got {stats.ignored_count}"
            )

            # Verify total calculation matches manual count
            expected_total = (
                0 * 2 + 3 + 0 + 5
            )  # 0 ready * 2 + 3 unpaired G + 0 unpaired C + 5 ignored = 8
            ...
            unresolved_files = [
                f
                for f in notfound_files
                if "QC_FAILED" not in f.name and "NO_INITIAL" not in f.name
            ]

            assert len(ready_files) == 3, (
                f"Expected 3 ready files in directory, got {len(ready_files)}"
            )
            assert len(unresolved_files) == 2, (
                f"Expected 2 unresolved files in directory, got {len(unresolved_files)}"
            )
            assert len(qc_failed_files) == 2, (
                f"Expected 2 QC failed files in directory, got {len(qc_failed_files)}"
            )
            assert len(no_initial_files) == 1, (
                f"Expected 1 non-initial file in directory, got {len(no_initial_files)}"
            )
    ...
    def test_clinical_data_batch_accuracy(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir_str:
            ...
            csv_paths = self.create_realistic_csv_data(
                temp_dir, "clinical_batch", "clinical_data_batch"
            )
            case_id_mapping = self.create_realistic_case_id_mapping(
                "clinical_data_batch"
            )
            ...
            assert stats.unpaired_genomic_count == 0, (
                f"Expected 0 unpaired genomic files, got {stats.unpaired_genomic_count}"
            )
            assert stats.unpaired_clinical_count == 2, (
                f"Expected 2 unpaired clinical files, got {stats.unpaired_clinical_count}"
            )
            assert stats.ignored_count == 4, (
                f"Expected 4 ignored files, got {stats.ignored_count}"
            )
            ...
            expected_total = (
                0 * 2 + 0 + 2 + 4
            )  # 0 ready pairs * 2 + 0 unpaired G + 2 unpaired C + 4 ignored = 6
            actual_total = stats.get_total_files()
            assert actual_total == expected_total, (
                f"Expected total {expected_total}, got {actual_total}"
            )
    ...
    def test_mixed_realistic_batch_accuracy(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir_str:
            ...
            csv_paths = self.create_realistic_csv_data(
                temp_dir, "mixed_batch", "mixed_realistic_batch"
            )
            case_id_mapping = self.create_realistic_case_id_mapping(
                "mixed_realistic_batch"
            )
            ...
            assert stats.ready_pairs_count == 2, (
                f"Expected 2 ready pairs, got {stats.ready_pairs_count}"
            )
            assert stats.unpaired_genomic_count == 0, (
                f"Expected 0 unpaired genomic files, got {stats.unpaired_genomic_count}"
            )
            ...
            assert stats.ignored_count == 6, (
                f"Expected 6 ignored files, got {stats.ignored_count}"
            )

            # Verify total calculation
            expected_total = (
                2 * 2 + 0 + 0 + 6
            )  # 2 ready * 2 + 0 unpaired G + 0 unpaired C + 6 ignored = 10
            actual_total = stats.get_total_files()
            assert actual_total == expected_total, (
                f"Expected total {expected_total}, got {actual_total}"
            )
            ...
            assert abs(total_proportion - 1.0) < 0.01, (
                f"Proportions should sum to 1.0, got {total_proportion}"
            )
    ...
    @patch("mvh_copy_mb.cli.create_gepado_client_from_env")
    @patch("mvh_copy_mb.cli.validate_and_update_record")
    def test_gepado_statistics_accuracy_with_realistic_data(
        self, mock_validate_update, mock_create_client
    ):
        ...
        with tempfile.TemporaryDirectory() as temp_dir_str:
            ...
            csv_paths = self.create_realistic_csv_data(
                temp_dir, "gepado_batch", "mixed_realistic_batch"
            )
            case_id_mapping = self.create_realistic_case_id_mapping(
                "mixed_realistic_batch"
            )
            ...
            assert stats.gepado_genomic_updates == 2, (
                f"Expected 2 GEPADO genomic updates, got {stats.gepado_genomic_updates}"
            )
            assert stats.gepado_clinical_updates == 1, (
                f"Expected 1 GEPADO clinical update, got {stats.gepado_clinical_updates}"
            )
            assert stats.gepado_errors == 1, (
                f"Expected 1 GEPADO error, got {stats.gepado_errors}"
            )
        ...
        with tempfile.TemporaryDirectory() as temp_dir_str:
            ...
            for i, row in enumerate(large_batch_rows):
                ...
                with open(csv_path, "w", newline="", encoding="utf-8") as f:
                    writer = csv.DictWriter(
                        f,
                        fieldnames=["Vorgangsnummer", "Meldebestaetigung"],
                        delimiter=";",
                    )
                    writer.writeheader()
                    writer.writerow(row)
            ...
            assert stats.ready_pairs_count == 25, (
                f"Expected 25 ready pairs, got {stats.ready_pairs_count}"
            )
            assert stats.unpaired_genomic_count == 0, (
                f"Expected 0 unpaired genomic files, got {stats.unpaired_genomic_count}"
            )
            ...
            assert stats.ignored_count == 45, (
                f"Expected 45 ignored files, got {stats.ignored_count}"
            )

            # Verify total calculation for large batch
            expected_total = (
                25 * 2 + 0 + 0 + 45
            )  # 25 ready pairs * 2 + 0 unpaired G + 0 unpaired C + 45 ignored = 95
            actual_total = stats.get_total_files()
            assert actual_total == expected_total, (
                f"Expected total {expected_total}, got {actual_total}"
            )
    ...
    def test_edge_case_statistics_accuracy(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir_str:
            ...
            assert stats.unpaired_genomic_count == 1, (
                f"Expected 1 unpaired genomic file, got {stats.unpaired_genomic_count}"
            )
            assert stats.unpaired_clinical_count == 0, (
                f"Expected 0 unpaired clinical file, got {stats.unpaired_clinical_count}"
            )
            assert stats.ignored_count == 3, (
                f"Expected 3 ignored files, got {stats.ignored_count}"
            )
            ...
            expected_total = (
                0 * 2 + 1 + 0 + 3
            )  # 0 ready pairs * 2 + 1 unpaired G + 0 unpaired C + 3 ignored = 4
            actual_total = stats.get_total_files()
            assert actual_total == expected_total, (
                f"Expected total {expected_total}, got {actual_total}"
            )
            ...
            assert stats_zero.ready_pairs_count == 0, (
                f"Expected 0 ready pairs, got {stats_zero.ready_pairs_count}"
            )
            assert stats_zero.unpaired_genomic_count == 0, (
                f"Expected 0 unpaired genomic files, got {stats_zero.unpaired_genomic_count}"
            )
            assert stats_zero.unpaired_clinical_count == 0, (
                f"Expected 0 unpaired clinical files, got {stats_zero.unpaired_clinical_count}"
            )
            assert stats_zero.ignored_count == 2, (
                f"Expected 2 ignored files, got {stats_zero.ignored_count}"
            )
