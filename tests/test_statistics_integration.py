...
def test_statistics_appear_correctly_on_page_load(test_db, monkeypatch):
    ...
    expected_stats = [
        ...
        (
            "Ready:",
            "readyPairs || '--'",
        ),  # Note: template uses 'Ready' instead of 'Complete'
        ("Done:", "donePairs || '--'"),
    ]
    ...
    stats_container_in_parent = filter_stats_container.find(
        "div", class_="statistics-container"
    )

    assert filter_container is not None, "Filter container should be present"
    assert stats_container_in_parent is not None, (
        "Statistics container should be in filter-stats container"
    )
...
def test_statistics_update_when_done_status_changes(monkeypatch):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        toggle_pair = next(
            (p for p in pairs_data1 if p["case_id"] == "CASE_TOGGLE"), None
        )
        ...
        response2 = client.post("/api/done/CASE_TOGGLE", data={"done": "true"})
        ...
        with MeldebestaetigungDatabase(db_path) as db:
            ...
            assert updated_genomic.is_done is True, (
                "Genomic record should be marked as done"
            )
            assert updated_clinical.is_done is True, (
                "Clinical record should be marked as done"
            )
        ...
        updated_toggle_pair = next(
            (p for p in pairs_data3 if p["case_id"] == "CASE_TOGGLE"), None
        )
        ...
        assert stats_container is not None, (
            "Statistics container should still be present"
        )
