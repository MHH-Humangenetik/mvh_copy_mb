    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for pair in pairs:
            if pair.case_id in seen_case_ids:
                ...
                pytest.fail(
                    f"Case ID {pair.case_id} appears multiple times in the output"
                )
...
def test_priority_group_1_contains_complete_pairs_not_done(
    num_pairs: int, done_flags: list
):
    """
...
def test_priority_group_2_contains_incomplete_pairs(
    num_incomplete: int, has_genomic_flags: list
):
    """
...
def test_priority_group_3_contains_complete_pairs_done(
    num_pairs: int, done_flags: list
):
    """
...
def test_done_status_update_affects_both_records_in_pair(
    case_id: str, initial_done: bool, updated_done: bool
):
    """
...
def test_records_sorted_by_priority_then_case_id(
    num_complete_not_done: int, num_incomplete: int, num_complete_done: int
):
    """
...
@given(
    ...
    record_types=st.lists(
        st.integers(min_value=0, max_value=2), min_size=1, max_size=15
    ),
)
def test_displayed_records_match_database_state(num_case_ids: int, record_types: list):
    """
        ...
        for created_record in records_with_case_id:
            ...
            displayed_record = next(
                (
                    ...
                    if r.vorgangsnummer == created_record.vorgangsnummer
                ),
                None,
            )
            ...
            assert (
                displayed_record.meldebestaetigung == created_record.meldebestaetigung
            )
            ...
            assert (
                displayed_record.indikationsbereich == created_record.indikationsbereich
            )
