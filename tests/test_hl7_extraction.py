...
class TestHL7ExtractionProperties:
    ...
    def test_hl7_id_extraction_consistency(
        self, case_id, prefix_text, suffix_text, letter_code
    ):
        """
    ...
    @given(
        ...
        letter_code1=st.text(
            alphabet=st.characters(whitelist_categories=("Lu", "Ll")),
            min_size=1,
            max_size=10,
        ),
        letter_code2=st.text(
            alphabet=st.characters(whitelist_categories=("Lu", "Ll")),
            min_size=1,
            max_size=10,
        ),
    )
    def test_multiple_patterns_first_extracted(
        self, case_id1, case_id2, middle_text, letter_code1, letter_code2
    ):
        """
        ...
        if (
            not middle_text
            or middle_text.strip() == ""
            or any(c.isalnum() for c in middle_text[:1])
            or "_" in middle_text
        ):
            middle_text = " "
    ...
    @given(text_without_pattern=st.text().filter(lambda x: "HUMGEN_" not in x))
    ...
    def test_hl7_pattern_handling_edge_cases(
        self, text_content, num_patterns, case_ids, letter_codes
    ):
        """
        ...
        **Validates: Requirements 3.3, 3.4, 3.5**
        ...
        patterns = [
            f"HUMGEN_{letter_codes[i]}_{case_ids[i]}" for i in range(num_patterns)
        ]
        ...
        assert result == case_ids[0], (
            f"Expected first case ID {case_ids[0]}, got {result}"
        )
