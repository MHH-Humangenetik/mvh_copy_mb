...
class TestHL7ExtractionProperties:
    ...
    @given(
        ...
        letter=st.text(alphabet=st.characters(whitelist_categories=("Lu", "Ll")), min_size=1, max_size=1)
    )
    def test_hl7_id_extraction_consistency(self, case_id, prefix_text, suffix_text, letter):
        """
        ...
        **Validates: Requirements 3.1, 3.2**
        ...
        meldebestaetigung = f"{prefix_text}HUMGEN_{letter}_{case_id}{suffix_text}"
    ...
    @given(
        ...
        letter1=st.text(alphabet=st.characters(whitelist_categories=("Lu", "Ll")), min_size=1, max_size=1),
        letter2=st.text(alphabet=st.characters(whitelist_categories=("Lu", "Ll")), min_size=1, max_size=1)
    )
    def test_multiple_patterns_first_extracted(self, case_id1, case_id2, middle_text, letter1, letter2):
        """
        ...
        meldebestaetigung = f"HUMGEN_{letter1}_{case_id1}{middle_text}HUMGEN_{letter2}_{case_id2}"
    ...
    @given(
        ...
        letters=st.lists(
            st.text(alphabet=st.characters(whitelist_categories=("Lu", "Ll")), min_size=1, max_size=1),
            min_size=2, max_size=5
        )
    )
    def test_hl7_pattern_handling_edge_cases(self, text_content, num_patterns, case_ids, letters):
        """
        ...
        **Validates: Requirements 3.3, 3.4, 3.5**
        """
        # Ensure we have enough case_ids and letters for the number of patterns
        if len(case_ids) < num_patterns:
            case_ids = case_ids * ((num_patterns // len(case_ids)) + 1)
        if len(letters) < num_patterns:
            letters = letters * ((num_patterns // len(letters)) + 1)
        
        case_ids = case_ids[:num_patterns]
        letters = letters[:num_patterns]
        
        # Test case 1: Multiple patterns - should extract first
        patterns = [f"HUMGEN_{letters[i]}_{case_ids[i]}" for i in range(num_patterns)]
        ...
        invalid_pattern = f"{text_content} HUMGEN_{letters[0]}_"
