...
class TestCaseValidationResult:
    ...
    def test_case_validation_result_defaults(self):
        ...
        result = CaseValidationResult(original_case_id="12345", is_valid=False)
...
class TestCorrectHL7CaseIdForGepado:
    ...
    def test_successful_correction(self, mock_validate, mock_get_guid, mock_find_case):
        ...
        mock_find_case.assert_called_once_with(
            client, "patient-guid-123", "incorrect_case_123"
        )
    ...
    @patch("mvh_copy_mb.hl7_case_id_correction.get_patient_guid_for_case")
    @patch("mvh_copy_mb.hl7_case_id_correction.validate_hl7_case_sapvisitingtype")
    def test_correction_fails_no_correct_case(
        self, mock_validate, mock_get_guid, mock_find_case
    ):
...
class TestHL7CaseIdCorrectionProperties:
    ...
    def test_property_sapvisitingtype_validation_accuracy(
        self, hl7_case_id, sap_visiting_type
    ):
        """
    ...
    def test_property_patient_guid_extraction_consistency(
        self, hl7_case_id, patient_guid
    ):
        """
    ...
    def test_property_correct_case_search_completeness(
        self, patient_guid, genomic_cases
    ):
        """
    ...
    def test_property_correct_case_passthrough(self, hl7_case_id, is_correct_case):
        ...
        **Validates: Requirements 1.2**
        """
        with patch(
            "mvh_copy_mb.hl7_case_id_correction.validate_hl7_case_sapvisitingtype"
        ) as mock_validate:
            ...
            if is_correct_case:
                ...
                assert (
                    result == hl7_case_id
                )  # Falls back to original on correction failure
                mock_validate.assert_called_once_with(client, hl7_case_id)
    ...
    @given(
        ...
        sap_visiting_type=st.text(
            min_size=1,
            max_size=10,
            alphabet=st.characters(
                blacklist_categories=["Cs"], blacklist_characters=["\x00"]
            ),
        ).filter(lambda x: x != "GS"),
    )
