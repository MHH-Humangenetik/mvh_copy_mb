...
@settings(max_examples=100)
@given(num_complete_pairs=st.integers(min_value=1, max_value=10))
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for pair in pairs:
            ...
            if pair.is_complete:
                ...
                assert len(rows) == 2, (
                    f"Complete pair {pair.case_id} should have 2 rows"
                )
                ...
                priority_classes = [
                    ...
                    if "priority-group" in c
                ]
                assert len(set(priority_classes)) == 1, (
                    "Both rows should have same priority group"
                )
            else:
                # Should have 1 row
                assert len(rows) == 1, (
                    f"Incomplete pair {pair.case_id} should have 1 row"
                )
...
def test_complete_pair_indicator_is_conditional(num_complete: int, num_incomplete: int):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for pair in pairs:
            ...
            assert len(complete_indicators) > 0, (
                f"Complete indicator not found for {pair.case_id}"
            )
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for pair in pairs:
            ...
            assert len(valid_indicators) > 0, (
                f"Valid indicator not found for {pair.case_id}"
            )
...
def test_client_side_sort_orders_by_column(num_pairs: int, sort_direction: str):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        sorted_pairs = sorted(
            pairs,
            key=lambda p: (p.priority_group, p.case_id),
...
def test_filtering_and_sorting_preserve_pair_grouping(
    num_pairs: int, filter_str: str, sort_direction: str
):
    """
...
def test_property_chronological_sorting(num_pairs: int, sort_direction: str):
    ...
    **Validates: Requirements 4.3**
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        sorted_pairs = sorted(
            pairs, key=get_sort_key, reverse=(sort_direction == "desc")
        )
        ...
        null_pairs = [
            ...
            if (p.genomic and p.genomic.output_date is None)
            or (p.clinical and p.clinical.output_date is None)
        ]
