...
class TestLeistungsdatumExtraction:
    ...
    def test_parse_leistungsdatum_basic(self):
        ...
        assert (
            parse_leistungsdatum("A123456789&2024070100&rest") is None
        )  # Wrong length
        assert parse_leistungsdatum("A123456789&2024130100A&rest") is None  # Non-digit
        assert (
            parse_leistungsdatum("A123456789&20241301001&rest") is None
        )  # Invalid month
        assert (
            parse_leistungsdatum("A123456789&20240732001&rest") is None
        )  # Invalid day
...
class TestLeistungsdatumExtractionProperties:
    ...
    @given(
        meldebestaetigung_code=st.text(
            alphabet="ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789", min_size=10, max_size=10
        ),
        year=st.integers(min_value=2020, max_value=2030),
        month=st.integers(min_value=1, max_value=12),
        day=st.integers(
            min_value=1, max_value=28
        ),  # Use 28 to avoid month-specific day issues
    ...
    def test_hash_string_parsing_extracts_correct_field(
        self, meldebestaetigung_code, year, month, day, counter, remaining_fields
    ):
        """
    ...
    def test_date_portion_extraction(self, leistungsdatum_string):
        ...
        **Validates: Requirements 1.3**
        ...
        hash_string = (
            f"A123456789&{leistungsdatum_string}&260530103&KDKK00001&0&O&9&1&C&2&1"
        )
        ...
        if result is not None:
            ...
            try:
                ...
                assert False, (
                    f"Parser returned date for invalid date components: {year}-{month}-{day}"
                )
        ...
        if result is not None:
            ...
            alt_hash1 = (
                f"A123456789&{alt_leistungsdatum1}&260530103&KDKK00001&0&O&9&1&C&2&1"
            )
            alt_hash2 = (
                f"A123456789&{alt_leistungsdatum2}&260530103&KDKK00001&0&O&9&1&C&2&1"
            )
        ...
        **Validates: Requirements 1.4**
        ...
        for invalid_leistungsdatum in test_cases:
            ...
            assert result is None, (
                f"Expected None for invalid date {invalid_leistungsdatum}, got {result}"
            )

    @given(
        malformed_input=st.one_of(
            st.text(max_size=50).filter(
                lambda x: "&" not in x or len(x.split("&")) < 2
            ),  # No second field
            st.text(
    ...
    def test_malformed_input_resilience(self, malformed_input):
        ...
        **Validates: Requirements 1.5**
        ...
        try:
            ...
            assert result is None, (
                f"Expected None for malformed input '{malformed_input}', got {result}"
            )
        except Exception as e:
            # Should not raise any exceptions - graceful error handling
            assert False, (
                f"Function crashed with malformed input '{malformed_input}': {e}"
            )
        ...
        for case in malformed_cases:
            try:
                ...
                assert result is None, (
                    f"Expected None for malformed case '{case}', got {result}"
                )
            except Exception as e:
            ...
            try:
                ...
                if result is not None:
                    ...
                    assert isinstance(result, date), (
                        f"Expected date object or None, got {type(result)}"
                    )
            except Exception as e:
                # Should not raise exceptions
        ...
        **Validates: Requirements 6.1**
        ...
        for null_case in null_cases:
            ...
            try:
                result = parse_leistungsdatum(hash_string)
                assert result is None, (
                    f"Expected None for null case '{null_case}', got {result}"
                )
            except Exception as e:
                assert False, (
                    f"Function should handle NULL case gracefully, but crashed with '{null_case}': {e}"
                )
        ...
        try:
            ...
            assert False, (
                f"Function should handle None input gracefully, but crashed: {e}"
            )
        ...
        try:
            ...
            assert False, (
                f"Function should handle empty string gracefully, but crashed: {e}"
            )
        ...
        try:
            ...
            assert False, (
                f"Function should handle incomplete hash gracefully, but crashed: {e}"
            )
    ...
    def test_legacy_format_handling(
        self, meldebestaetigung_code, year, month, day, counter, remaining_fields
    ):
        """
        ...
        **Validates: Requirements 6.3**
        ...
        current_fields = [
            meldebestaetigung_code,
            current_leistungsdatum,
        ] + remaining_fields
        current_hash = "&".join(current_fields)

        try:
            current_result = parse_leistungsdatum(current_hash)
            assert current_result == date(year, month, day), (
                "Current format should parse successfully"
            )
        except Exception as e:
        ...
        try:
            ...
            assert legacy_result2 is None, (
                "Legacy format with different structure should return None"
            )
        except Exception as e:
            assert False, f"Legacy format should not crash: {e}"
        ...
        for legacy_date in legacy_date_formats:
            ...
            try:
                ...
                assert legacy_result3 is None, (
                    f"Legacy date format '{legacy_date}' should return None"
                )
            except Exception as e:
                assert False, (
                    f"Legacy date format '{legacy_date}' should not crash: {e}"
                )
        ...
        for problematic_input in problematic_inputs:
            try:
                ...
                assert result is None, (
                    f"Problematic input '{problematic_input}' should return None"
                )
            except Exception as e:
                assert False, (
                    f"Processing should continue despite failure with '{problematic_input}': {e}"
                )
