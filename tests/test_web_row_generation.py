...
def test_complete_pair_column_count():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert len(genomic_cols) == 11, (
            f"Genomic row should have 11 columns, got {len(genomic_cols)}"
        )

        # Clinical row: 7 columns (4 spanned from genomic)
        clinical_row = rows[1]
        clinical_cols = clinical_row.find_all("td")
        assert len(clinical_cols) == 7, (
            f"Clinical row should have 7 columns, got {len(clinical_cols)}"
        )

        # Verify rowspan attributes on genomic row
        assert genomic_cols[0].get("rowspan") == "2", "Case ID should have rowspan=2"
        assert genomic_cols[8].get("rowspan") == "2", (
            "Complete indicator should have rowspan=2"
        )
        assert genomic_cols[9].get("rowspan") == "2", (
            "Valid indicator should have rowspan=2"
        )
...
def test_genomic_only_pair_has_one_row():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert len(cols) == 11, (
            f"Genomic-only row should have 11 columns, got {len(cols)}"
        )
...
def test_clinical_only_pair_has_one_row():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        cols = rows[0].find_all("td")
        assert len(cols) == 11, (
            f"Clinical-only row should have 11 columns, got {len(cols)}"
        )
...
def test_complete_pair_has_checkbox():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert checkboxes[0].has_attr("checked"), (
            "Checkbox should be checked when is_done=True"
        )
