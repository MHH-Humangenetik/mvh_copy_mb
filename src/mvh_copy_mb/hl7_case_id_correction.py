...
def validate_hl7_case_sapvisitingtype(client: GepadoClient, hl7_case_id: str) -> bool:
    ...
    try:
        ...
        if row:
            ...
            logger.debug(
                f"HL7 case ID {hl7_case_id} sapVisitingType: {sap_visiting_type} (valid: {is_valid})"
            )
        ...
        logger.error(
            f"Failed to validate sapVisitingType for HL7 case ID {hl7_case_id}: {e}"
        )
        raise
    finally:
        if cursor:
...
def find_correct_genomic_case(
    client: GepadoClient, patient_guid: str, original_case_id: str = None
) -> Optional[str]:
    """
    ...
    try:
        ...
        if len(rows) == 0:
            ...
            if original_case_id:
                logger.warning(
                    f"No genomic sequencing cases found for patient GUID {patient_guid} (original case ID: {original_case_id})"
                )
            else:
                logger.warning(
                    f"No genomic sequencing cases found for patient GUID {patient_guid}"
                )
            return None
        elif len(rows) == 1:
            corrected_case_id = rows[0][0]
            logger.info(
                f"Found single correct genomic case {corrected_case_id} for patient GUID {patient_guid}"
            )
            ...
            if original_case_id:
                logger.error(
                    f"Multiple genomic sequencing cases found for patient GUID {patient_guid} (original case ID: {original_case_id}). "
                    f"Manual review required for candidates: {', '.join(case_ids)}"
                )
            else:
                logger.error(
                    f"Multiple genomic sequencing cases found for patient GUID {patient_guid}. "
                    f"Manual review required for candidates: {', '.join(case_ids)}"
                )
            return None
        ...
        logger.error(
            f"Failed to find correct genomic case for patient GUID {patient_guid}: {e}"
        )
        raise
    finally:
        if cursor:
...
def correct_hl7_case_id_for_gepado(client: GepadoClient, hl7_case_id: str) -> str:
    ...
    try:
        ...
        if is_valid:
            ...
            logger.info(
                f"HL7 case ID validation PASSED for {hl7_case_id} - sapVisitingType is 'GS', no correction needed"
            )
            return hl7_case_id
        else:
            # Log the validation failure with specific reason
            logger.warning(
                f"HL7 case ID validation FAILED for {hl7_case_id} - sapVisitingType is not 'GS', attempting correction"
            )
        ...
        if not patient_guid:
            logger.error(
                f"Cannot correct HL7 case ID {hl7_case_id}: patient GUID not found in av_ordermanagement"
            )
            return hl7_case_id

        logger.info(
            f"Found patient GUID {patient_guid} for HL7 case ID {hl7_case_id}, searching for correct genomic case"
        )
        ...
        if corrected_case_id:
            ...
            logger.info(
                f"HL7 case ID correction SUCCESSFUL: {hl7_case_id} -> {corrected_case_id} (patient GUID: {patient_guid})"
            )
            return corrected_case_id
        else:
            ...
            logger.warning(
                f"HL7 case ID correction FAILED for {hl7_case_id}: returning original case ID"
            )
            return hl7_case_id
        ...
        if "pymssql" in str(type(e)) or "database" in str(e).lower():
            logger.error(
                f"Database connection/query error during HL7 case ID correction for {hl7_case_id}: {type(e).__name__}: {e}"
            )
        else:
            logger.error(
                f"Unexpected error during HL7 case ID correction for {hl7_case_id}: {type(e).__name__}: {e}"
            )
        return hl7_case_id
