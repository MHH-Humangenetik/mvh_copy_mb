...
def parse_leistungsdatum(hash_string: Optional[str]) -> Optional[date]:
    ...
    try:
        ...
        if len(fields) < 2:
            logger.warning(
                f"Hash string has insufficient fields: {len(fields)}, expected at least 2"
            )
            return None
        ...
        if len(leistungsdatum_field) != 11:
            logger.warning(
                f"Leistungsdatum field has invalid length: {len(leistungsdatum_field)}, expected 11"
            )
            return None
        ...
        if not leistungsdatum_field.isdigit():
            logger.warning(
                f"Leistungsdatum field contains non-digit characters: {leistungsdatum_field}"
            )
            return None
