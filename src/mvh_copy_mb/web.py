...
async def index(request: Request):
    ...
    try:
        ...
        pairs_dict = []
        for pair in pairs:
            # Convert records to dicts and handle datetime/date serialization
            genomic_dict = None
            if pair.genomic:
                genomic_dict = asdict(pair.genomic)
                if genomic_dict.get("processed_at"):
                    genomic_dict["processed_at"] = genomic_dict[
                        "processed_at"
                    ].isoformat()
                if genomic_dict.get("output_date"):
                    genomic_dict["output_date"] = genomic_dict[
                        "output_date"
                    ].isoformat()
            ...
            if pair.clinical:
                ...
                if clinical_dict.get("processed_at"):
                    clinical_dict["processed_at"] = clinical_dict[
                        "processed_at"
                    ].isoformat()
                if clinical_dict.get("output_date"):
                    clinical_dict["output_date"] = clinical_dict[
                        "output_date"
                    ].isoformat()
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to load records from database: {str(e)}"
        )
...
async def update_done_status(case_id: str, request: Request):
    ...
    try:
        ...
        if not db_path.exists():
            ...
            raise HTTPException(
                status_code=500, detail=f"Database not found at {db_path}"
            )
        ...
        try:
            ...
            if not result:
                raise HTTPException(
                    status_code=500, detail="Failed to update done status"
                )
            ...
            if pair is None:
                raise HTTPException(
                    status_code=404, detail=f"Case ID {case_id} not found after update"
                )

            # Render the updated pair rows for HTMX swap
            return templates.TemplateResponse(request, "pair_rows.html", {"pair": pair})
        ...
        logger.error(
            f"Error updating done status for Case ID {case_id}: {e}", exc_info=True
        )
        raise HTTPException(
            status_code=500, detail=f"Failed to update done status: {str(e)}"
        )
